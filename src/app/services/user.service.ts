...
export class UserService {
   ...
   private readonly baseURL:string="http://localhost:6837/api/ApplicationUser/"
...
'Authorization':`Bearer ${userInfo?.token }`
    ...
    return this.httpClient.get<ResponseModel>(this.baseURL+"GetUser",{headers:headers}).pipe(map(res=>{
      let userList=new Array<User>();
      if(res.responseCode==ResponseCode.OK)
      {
           if(res.dateSet)
